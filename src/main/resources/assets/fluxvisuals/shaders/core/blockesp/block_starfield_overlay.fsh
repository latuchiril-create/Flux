...
void main() {
    time = GameTime * LineWidth * 240.0;
    screenSize = max(ScreenSize, vec2(1.0));
    ...
    float finalAlpha = mix(alpha * 0.4, 1.0, smoothstep(0.98, 1.0, alpha));
    fragColor = vec4(result.rgb, finalAlpha) * ColorModulator;
}
